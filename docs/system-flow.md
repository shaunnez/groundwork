...
- Larger packs: extract once per content hash, create token-bounded units, analyse changed documents only, cache source-backed findings and synthesise from compact findings with source lookups. Exhaustive requirements work continues to process all units in its declared scope. Selective questions use Postgres lexical retrieval first. Add vector/hybrid retrieval only when a measured missed-evidence case justifies it; do not add another database by default.
